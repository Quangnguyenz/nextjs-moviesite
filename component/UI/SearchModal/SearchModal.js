...
const SearchModal = (props) => {
    ...
    const handleInput = async (e) => {
        try {
            ...
            let searchData = await axios.get(`https://api.themoviedb.org/3/search/multi?query=${e.target.value}&api_key=1f3708ab08ebaadf0ef295924a2b4ac3&language=en-US`);
    ...
    return (
        <div className={`search-modal ${globalState.searchOpen ? "search-modal--active" : ""}`}>
            ...
            <h3 className="search-modal__title">
                {showResults && searchData.length >= 1 ? `Search Result for ${text}` : 'Popular Searches'}
            </h3>

            <div className="search-modal__thumbnails">
                {showResults && searchData.length >= 1 ? <SearchResults searchData={searchData} clickedThumbnail={clickedThumbnail} /> : <PopularResults popData={popData} clickedThumbnail={clickedThumbnail} />}
            </div>
        </div>
    )
}
...
const PopularResults = (props) => {
    return props.popData.map((item, index) => {
        return (
            <div
                key={index}
                className="search-modal__thumbnail" onClick={() => props.clickedThumbnail('popular', item.id)}>
...
const SearchResults = (props) => {
    return props.searchData.map((item, index) => {
        return (
            <div key={index}
                className="search-modal__thumbnail" onClick={() => props.clickedThumbnail('search', item.id, item.media_type)}>
                <img src={`https://image.tmdb.org/t/p/w185/${item.poster_path}`} />
                <div className="search-modal__top-layer">
                    <i className="fas fa-play" />
                </div>
